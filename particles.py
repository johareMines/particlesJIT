        ...
        for j in range(num_particles):
            if i != j:
                ...
                if dist > 0:
                    ...
                    other_type = types[j]
                    if dist < min_distances[p_type, other_type]:
                        force = abs(forces[p_type, other_type]) * -3 * (1 - dist / min_distances[p_type, other_type]) * K
                        total_force_x += dir_x * force
                        total_force_y += dir_y * force
                        
                    if dist < radii[p_type, other_type]:
                        force = forces[p_type, other_type] * (1 - dist / radii[p_type, other_type]) * K
                        total_force_x += dir_x * force
                        total_force_y += dir_y * force
...
def set_parameters():
    ...
    print(f"Forces {forces} | minDist {min_distances} | Radii {radii}")
